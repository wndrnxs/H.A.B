...
| **자산** | 순자산 추이, 갚는 중인 대출, 계좌별 잔액, 자산 구성, 분류별 지출(도넛), 결제수단별, 최근 12개월 수입·지출 |
| **설정** | 고정지출 자동 등록, 분류·예산·계좌 편집, 백업 내보내기/불러오기, 테마 |

그 밖에:

- **고정지출 자동 등록** — 월세·통신비·보험료처럼 매달 같은 날 나가는 것을 등록해 두면
  그 날이 지날 때 알아서 적힌다. 거래 id 를 `r_<항목>_<YYYY-MM>` 으로 정해 두어 두
  사람이 동시에 열어도 한 건으로 합쳐지고, 지운 내역을 다시 만들어 내지도 않는다.
  급여나 적금 자동이체처럼 수입·이체도 등록할 수 있다.
...
- **순자산과 대출 상환** — 원금을 갚으면 통장이 줄고 빚도 같은 만큼 줄어, **순자산은
  그대로**다. 늘어나는 건 `수입 − 지출` 뿐이고 이체는 순자산을 바꾸지 않는다.
  대신 대시보드의 `자산으로 돌린 돈` 이 적금·투자 입금과 대출 원금상환을 합쳐
  보여 준다 — 쓰지 않고 내 쪽에 남긴 돈이다. 카드대금은 뺀다(이미 쓴 시점에
  지출로 한 번 세었다).
- **숨긴 계좌** — 설정에서 `장부에 표시` 를 끄면 대시보드와 자산 화면의 순자산
  계산에서 빠진다. 전세보증금처럼 크고 움직이지 않는 것이 매달의 변화를 덮을 때
  쓴다. 자산 화면 맨 아래 `숨긴 계좌` 에 합계가 남아 없던 일이 되지는 않는다.
